    ...
    def toggle_detection(self):
        ...
        if not self.detection_running:
            ...
            try:
                cmd = [
                    "python", 
                    "./basic_pipelines/filter.py", 
                    "--input", "/home/team206/hailo-rpi5-examples/resources/PXL_20250206_222115020.mp4", 
                    "--hef", self.selected_model,
                    "--labels-json", "resources/test-labels.json",
                ]
    ...
    def toggle_recording(self):
        ...
        if not self.recording:
            ...
            if not self.detection_running:
                ...
                wait_time = 0
                while self.current_frame is None and wait_time < 3:
                    print("Waiting for frame...")
                    time.sleep(0.1)
                    wait_time += 0.1

                if self.current_frame is None:
                    self.status_label.config(text="Status: No frame available for recording")
                    print("ERROR: No current_frame available. Recording aborted.")
                    return
                ...
                try:
                    ...
                    print(f"Starting recording: {output_file} ({width}x{height})")
                    ...
                    # Optional: Start a timer display thread
                    def update_timer():
