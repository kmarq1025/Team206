...
class RaspberryPiCameraApp:
    ...
    def toggle_recording(self):
        ...
        if not self.recording[0]:
            ...
            if not self.detection_running[0]:
                ...
                # Configuration
                CAPTURES_DIR = "recordings"
                os.makedirs(CAPTURES_DIR, exist_ok=True)
                FRAME_RATE = 10  # Adjust as needed
                TOTAL_FRAMES = FRAME_RATE * 5  # Capture for 5 seconds
                VIDEO_NAME = os.path.join(CAPTURES_DIR, "capture_video.avi")

                # Get screen dimensions
                region = (1, 1, 1520, 1020)
                ...
                for _ in range(TOTAL_FRAMES):
                    ...
                    out.write(frame)
                    time.sleep(1 / FRAME_RATE)  # Control capture speed
    ...
    def capture_still(self):
        ...
        if not self.detection_running[0]:
            ...
            output_file = os.path.join(CAPTURES_DIR, f"capture_{timestamp}.jpg")
            screenshot = pyautogui.screenshot(region=(1,1,1520,1020)) # take a screenshot of the active window
            frame_still = np.array(screenshot)
            frame_still = cv2.cvtColor(frame_still, cv2.COLOR_RGB2BGR);
            cv2.imwrite(output_file, frame_still)
